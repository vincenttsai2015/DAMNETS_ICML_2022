...
#
#   bash deploy.sh [tarball]        預設 ~/damnets_deploy.tar.gz
#
# 為什麼不直接 tar xzf：
...
TARBALL=${1:-~/damnets_deploy.tar.gz}
DEST=~/DAMNETS_ICML_2022
...
echo
echo "--- 重點檔案 ---"
ls -l --time-style=+%H:%M "$DEST/run_damnets.sh" "$DEST/run_damnets_pipeline.sh" \
                          "$DEST/submit_damnets.sh" "$DEST/models/dmnets_gnn_mb.py" 2>/dev/null || true
